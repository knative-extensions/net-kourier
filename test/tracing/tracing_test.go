...
func TestTracingProtocols(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.protocol, func(t *testing.T) {
			...
			startTime := time.Now()
			name, _, client, ing := createTestIngressAndService(ctx, t, clients)

			makeRequest(t, client, "http://"+name+"."+test.NetworkingFlags.ServiceDomain)
			...
			numberOfTracesFromHTTPCalls := queryJaegerTraces(ctx, t, clients,
				map[string]string{
					...
					"http.url":         "http://" + name + "." + test.NetworkingFlags.ServiceDomain + "/",
			...
			numberOfTracesFromKnativeIngressProbe := queryJaegerTraces(ctx, t, clients,
				map[string]string{
					"http.status_code": "200",
					"http.url":         "http://" + name + "." + test.NetworkingFlags.ServiceDomain + "/healthz",
					"upstream_cluster": testsNamespace + "/" + name,
...
func TestTracingDisabled(t *testing.T) {
	...
	startTime := time.Now()
	name, _, client, _ := createTestIngressAndService(ctx, t, clients)
...
func TestTracingSamplingRate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			url := "http://" + name + "." + test.NetworkingFlags.ServiceDomain
...
func createTestIngressAndService(ctx context.Context, t *testing.T, clients *test.Clients) (name string, port int, client *http.Client, ing *v1alpha1.Ingress) {
	t.Helper()
