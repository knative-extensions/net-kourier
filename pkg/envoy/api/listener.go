...
func NewHTTPListener(manager *hcm.HttpConnectionManager, port uint32, enableProxyProtocol bool, address string) (*listener.Listener, error) {
	...
	return &listener.Listener{
		...
		Address:         createAddress(port, address),
...
func NewHTTPSListener(port uint32, filterChain []*listener.FilterChain, enableProxyProtocol bool, address string) (*listener.Listener, error) {
	var listenerFilter []*listener.ListenerFilter
	if enableProxyProtocol {
		proxyProtocolListenerFilter, err := createProxyProtocolListenerFilter()
		if err != nil {
