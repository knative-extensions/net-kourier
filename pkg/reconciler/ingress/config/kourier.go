...
func asTracing(tracing *Tracing) cm.ParseFunc {
	return func(data map[string]string) error {
		...
		} else if protocol != TracingProtocolHTTP && protocol != TracingProtocolGRPC {
			return fmt.Errorf("invalid tracing protocol %q, must be %q or %q", protocol, TracingProtocolHTTP, TracingProtocolGRPC)
		} else {
